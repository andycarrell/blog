    ...
    <Page>
      <Stack>
        ...
        <Paragraph>
          ...
          dependencies, using your project's package manager. In addition to
          these packages, there's some code to write and maintain within your
          project, which will be outlined in the article. Feel free to copy,
          update and change as you need.
        </Paragraph>
...
\``}
        ...
        <Paragraph>
          ...
          a change too. There are methods of fetching the schema dynamically  —
          which could be leveraged at run or build time  — this is on our
          backlog, but maintaining a static copy hasn't had a high enough cost
...
addMockFunctionsToSchema({ schema, mocks });`}
        ...
        <Quote>
          ...
          etc. So you can already get the right shape of result.
        </Quote>
        <Paragraph>
          ...
          this executable schema and have a mock straightaway. The mock wouldn't
          be particularly reliable or consistent, so isn't a great fit for
          testing yet.
        </Paragraph>
        <Paragraph>
          Conversely, the entire schema being mocked is extremely helpful for
          long term test maintenance  —  the suite has a built in resilience to
          (smaller) schema changes. Once the mocking utility is put together,
          and we've written tests that accurately define what they depend on, we
          have high confidence that tests only fail when the underlying code has
          changed.
        </Paragraph>
        ...
        <Paragraph>
          ...
          in each test, so that we can define the shape of the mock we need.
          This also allows us to only define the data we need, and encourages
          keeping tests independent. If you're not familiar with writing custom
          Cypress commands, check out the{" "}
          <ExternalLink href="https://docs.cypress.io/api/cypress-api/custom-commands.html#Syntax">
    ...
    cy.on("window:before:load", win => {
      ...
      cy.stub(win, "fetch").callsFake(fetch);
        ...
        <Paragraph>
          ...
          schema). This is less than ideal because we would have to use the same
          mock definition for every test.
        </Paragraph>
        <Paragraph>
...
// Cypress.Commands.add("mockGraphQLApi", ...)`}
        ...
        <Paragraph>
          ...
          mutation  — or a query that isn't a one to one mapping to a type  —
    ...
    updateAccount: () => ({
      address: () => ({
        ...
        <Paragraph>
          ...
          the second argument to our mock resolver. This is extremely useful for
    ...
    cy.on("window:before:load", win => {
      ...
      cy.stub(win, "fetch").callsFake(fetch);
        ...
        <Paragraph>
          ...
          as we need. The maintenance overhead compared to the value this adds
          has been an extremely good trade off for our team.
        </Paragraph>
        <Heading2>Conclusion</Heading2>
        <Paragraph>
          Using this simple mocking utility has enabled us to write a large
          number of high fidelity tests that run completely independently of our
          backend servers. In turn, this encourages good test coverage of
        ...
        <Paragraph>
          ...
          benefits we have. At each step I've explained what we did and why, but
          I appreciate information might be lacking  —  if so{" "}
